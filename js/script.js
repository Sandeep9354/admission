...
document.addEventListener('DOMContentLoaded', () => {
    ...
    profileModalEl.addEventListener('show.bs.modal', async () => {
        try {
            const res = await fetch('http://127.0.0.1:8000/api/me', { headers: authHeaders });
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/users/${userIdInput.value}`, {
                method: 'PUT',
                headers: authHeaders,
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            admissionForm.addEventListener("submit", async (e) => {
                ...
                try {
                    ...
                    // Submit to backend
                    const res = await fetch("http://127.0.0.1:8000/api/admissions", {
                        method: "POST",
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function fetchAdmissions() {
                ...
                try {
                    ...
                    const res = await fetch(`http://127.0.0.1:8000/api/admissions?${params.toString()}`, {
                        headers: authHeaders,
            ...
            async function showViewModal(admissionId) {
                ...
                try {
                    const res = await fetch(`http://127.0.0.1:8000/api/admissions/${admissionId}`, {
                        headers: authHeaders
                    });
            ...
            function populateViewModal(data) {
                ...
                const createFileLink = (filePath, fieldName) => {
                    ...
                    return `<a href="http://127.0.0.1:8000/${filePath}" target="_blank" class="file-link">${fileName}</a>`;
                };
